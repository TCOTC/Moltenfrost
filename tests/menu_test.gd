...
func _case_room_listed() -> bool:
	if _list().item_count == 0:
		if _stage_elapsed > WAIT_LIMIT:
			_fail("广播之后 %.1f 秒内列表里没有出现房间" % WAIT_LIMIT)
			return true
		return false
	_ok(_list().item_count == 1, "应当只列出一个房间，实际 %d 个" % _list().item_count)
	_ok(_list().get_item_text(0).contains(ROOM_NAME), "列表文案应当含房间名，实际「%s」" % _list().get_item_text(0))
	_list().select(0)
	# select() 不保证发出 item_selected，显式补一次，让选中状态与真人点击一致。
	_list().emit_signal("item_selected", 0)
	_ok(not _button("RoomButtons/Join").disabled, "选中房间之后加入按钮应当可用")
	_button("RoomButtons/Join").pressed.emit()
	_ok(_join_calls.size() == 1, "选中房间后点加入应当发出一次 join_requested，实际 %d 次" % _join_calls.size())
	if _join_calls.size() == 1:
		var address := String(_join_calls[0][0])
		_ok(not address.is_empty() and address != "0.0.0.0", "加入的地址应当是收到广播的那个地址，实际「%s」" % address)
		_ok(int(_join_calls[0][1]) == ROOM_PORT, "加入的端口应当取自房间而不是界面上的默认值，实际 %d" % int(_join_calls[0][1]))
	return true
...
func _case_actions() -> void:
	...
	_ok(_join_calls.size() == 2, "手动填地址后点加入应当再发一次 join_requested，实际 %d 次" % _join_calls.size())
	if _join_calls.size() == 2:
		_ok(String(_join_calls[1][0]) == "127.0.0.1", "手动加入的地址应当取自输入框，实际「%s」" % _join_calls[1][0])
		_ok(int(_join_calls[1][1]) == MENU_PORT, "手动加入的端口应当取自输入框，实际 %d" % int(_join_calls[1][1]))
